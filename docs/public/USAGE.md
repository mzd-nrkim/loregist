...
# 1. 환경변수 설정
cp .env.example .env
# STASHDEX_WORKSPACE: 문서·로그를 관리할 작업 루트 경로를 지정한다
...
> 프로젝트 디렉터리 밖(홈 등)에서 실행 시: 대화형 환경은 전체 프로젝트 검색 여부 프롬프트([Y/n]), 비대화형/파이프는 자동 전체검색(`--no-interactive`). 즉시 전체검색은 `--all-projects`.
...
2. `projects.toml`에 블록 추가 — 경로는 `STASHDEX_WORKSPACE` 기준 상대경로:
...
| 변수 | 기본값 | 설명 |
|------|--------|------|
| `STASHDEX_DB_PASSWORD` | `vector_local` | PostgreSQL 비밀번호 |
| `STASHDEX_WORKSPACE` | `~/workspace` | 작업 루트 경로 |
| `STASHDEX_NO_SSL_VERIFY` | `1` | 기업망 SSL inspection 우회 (1=활성) |

### STASHDEX_WORKSPACE 설정 방법

**방법 1 — 셸 프로파일 (`.zshrc` / `.bash_profile`)**

```bash
export STASHDEX_WORKSPACE=/절대/경로/workspace
```
...
{
  "env": {
    "STASHDEX_WORKSPACE": "/절대/경로/workspace"
  }
}
```
...
> `STASHDEX_WORKSPACE` 환경변수 설정은 [환경변수 절](#환경변수)을 참조.
