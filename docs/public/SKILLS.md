...
아래 캡처는 각 스킬을 **실제 `claude` CLI로 자연어 호출**해, 주제만 바꾼 3개 변형으로 실행한 결과다. 파일을 변경하는 스킬(`carry-over`/`handbook-update`/`wiki-update`)은 검증 목적상 모두 **dry-run**으로 실행해 작업 트리를 건드리지 않았다.

### 검색 (`stashdex search` 자동 호출)

방화벽·DB 권한·Airflow 세 주제로 검색하면, 단어가 아니라 의미로 관련 기록을 찾아 어느 파일에서 무엇을 찾았는지까지 정리한다.

![검색 — 방화벽/DB권한/Airflow 3개 주제 실행 결과](assets/verify-search-3topics.png)

### `add-work` — 작업 항목 등록

자연어로 항목 3개를 등록하면 인덱스에 체크박스+링크가 추가되고 주제별 문서가 생성된다(하단은 생성된 문서 원문 검증).

![add-work — 3개 주제 등록 + 생성 파일 내용](assets/verify-addwork-3topics.png)

### `daily-report` — 슬랙 보고 생성

아침/저녁/dual 세 모드로 실제 보고 본문을 생성하고 `daily-report.md`로 저장한다.

![daily-report — morning/evening/dual 보고 본문](assets/verify-dailyreport-3topics.png)

### `carry-over` — 미완료 이월 (dry-run)

자동 탐색·날짜 지정·미리보기 세 방식으로 이월될 항목 목록과 소스→대상 경로를 보여준다.

![carry-over — 3개 방식 이월 미리보기](assets/verify-carryover-3topics.png)

### `handbook-update` — 산문 문서 갱신 (dry-run)

README·ARCHITECTURE·USAGE 각각에 대해 stale 여부와 근거를 판정한다.

![handbook-update — README/ARCHITECTURE/USAGE stale 판정](assets/verify-handbook-3topics.png)

### `wiki-update` — handbook+catalog 통합 갱신 (dry-run)

여러 프로젝트에 걸쳐 handbook stale 수와 catalog 신규 topic/decision 수를 집계한다.

![wiki-update — 3개 프로젝트 dry-run 집계](assets/verify-wiki-3topics.png)
