...
| 토큰 절약 | Hot→Cold→vault 계층 + 시맨틱 검색으로 top-k 청크만 컨텍스트 주입 | stashdex search, 검색 계층 |
| LLM 핸드북 | LLM이 로그·코드 변경에서 산문 handbook(README·ARCHITECTURE)을 증류·유지. catalog로 자동 색인(opt-in) | handbook-update 스킬 |
...
graph LR
    ...
    B -->|"stashdex search<br/>top-k 청크 주입"| C["📖 LLM 핸드북<br/>산문 문서 증류·유지"]
...
flowchart LR
    ...
    subgraph A["✅ stashdex 사용"]
        direction TB
        Q2[쿼리] --> S["stashdex search\n시맨틱 검색"]
        S --> DB[(pgvector)]
        DB --> C2["컨텍스트 윈도우\n관련 청크 top-k\n노이즈 없음\n토큰 절약"]
    end
```
...
이 시스템이 동작하려면 **업무 활동이 텍스트 파일(`*.log` / `*.md` / `*.txt`)로 기록되어야** 한다. stashdex는 embed된 것만 검색할 수 있으므로, 기록되지 않은 업무는 검색되지 않는다.

| 기록 방법 | 설명 |
|---|---|
| `stashdex journal "메시지"` | 오늘 날짜 `.log`에 타임스탬프와 함께 append |
| `stashdex-journal.command` (더블클릭) | Finder에서 더블클릭 → 대화창에 텍스트 입력 → 기록. 터미널 불필요한 간편 진입점 |
| `stashdex watch` | 디렉터리 감시 — 파일 변경 시 자동 embed |
...
로그가 쌓이면 `stashdex embed`로 pgvector에 인덱싱한다. 이후 과거 이력은 전부 `stashdex search`로 소환 가능해진다.

## 유사 개념

stashdex를 이미 아는 개념으로 위치 짓는다면:
...
요약하면 stashdex는 **개인 업무 로그 위의 RAG + hot/cold 메모리 계층 + LLM이 유지하는 핸드북**의 결합이다.

### handbook과 LLM wiki(catalog) — 비슷하지만 다르다

Karpathy가 말한 **"LLM wiki"**는 LLM이 텍스트에서 지식을 *전자동*으로 증류·유지하는 지식 베이스다. stashdex의 catalog(`_wiki/`)가 여기에 대응한다 — 사람은 읽기만 하고, 생성·갱신은 LLM이 전담한다.
...
stashdex는 전자동 wiki보다 이 하이브리드를 메인으로 둔다 — 문서의 의도·구조는 사람이 통제하고, 갱신 비용만 LLM에 넘기는 균형이다.
...
요컨대 **handbook은 사람의 큐레이션으로 분량이 자연히 제한**되어 토큰 절약이 구조적으로 유지되지만, catalog는 자동 증식이라 상한이 없다. stashdex가 handbook을 메인, catalog를 opt-in 곁가지로 둔 이유다.

## 빠른 시작

코드·인프라 설정 없이 stashdex를 처음 쓰는 경우의 최소 흐름이다.

1. **프로젝트 등록** — `stashdex project add` 를 실행하면 대화형으로 프로젝트 키·경로를 입력받아 `projects.toml`에 자동 등록한다.
2. **업무 기록** — `stashdex journal "오늘 한 일"` 로 날짜별 로그 파일에 내용을 누적한다.
3. **과거 이력 검색** — `stashdex search "검색어"` 로 시맨틱 검색을 실행하면 관련 로그 청크를 상위 k개 반환한다.
...
handbook 산문 갱신(`handbook-update`)은 opt-in 없이 항상 동작한다. 그 위에 catalog 자동 색인(`catalog-update`)까지 활성화하려면 `projects.toml` 블록에 다음 필드를 추가한다.
...
flowchart TD
    ...
    REQ -->|과거 이력| SEARCH["② stashdex search\n시맨틱 검색 — Cold"]
...
각 프로젝트 repo의 CLAUDE.md에는 이 계층을 적용한 "문서·로그 컨텍스트" 규칙 블록이 들어간다(`*.log`, `cold/**` 기본 제외 + 과거 이력은 `stashdex search`로).
...
| ❄️ **Cold** | pgvector `doc_chunks` | `stashdex search` 시맨틱 검색 |
...
| `catalog-update` | `_wiki/` 읽기전용 인덱스 생성 — handbook(지정 시) 또는 작업문서·로그를 스캔해 topic·decision 파일 증류. `--recommend-sources`로 소스 추천, `--dry-run`으로 미리보기 |
...
> 설치 전체 절차(클론 → 보안 훅 → PATH → DB → 프로젝트 등록 → 간편 키트)는 [docs/public/SETUP.md](docs/public/SETUP.md)를 참조.
