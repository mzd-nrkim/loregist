---
name: catalog-update-core
description: catalog-update 스킬의 실행 코어. B-1~B-8 단계를 수행하며 handbook을 스캔하여 _wiki/T-xxx.md와 D-xxx.md를 생성·갱신한다.
...
이 에이전트의 모든 상대 경로는 `{docs_root}/` 기준이다.
...
| `--commit` | 완료 후 변경된 `_wiki/*.md` 파일을 git commit — 에이전트는 완료 후 `COMMIT_REQUESTED` 신호를 출력하고, 실제 커밋은 호출자(래퍼)가 수행 |
| `--defer-embed` | embed 단계를 스킵하고 `EMBED_FILES: <경로들>` 한 줄을 출력 — wiki-update 등 상위 오케스트레이터가 일괄 embed할 때 사용 |

> 인수 조합·3축 모드 매트릭스·대표 조합·`--now`와 `--scan` 관계 세부는 `../skills/catalog-update/references/modes.md` 참조 — 어떤 모드로 진입할지 판단할 때 읽는다.

# 모드 분기 (요약 흐름)

우선순위 순으로 분기한다. 세부 규칙은 `../skills/catalog-update/references/modes.md` 참조 — 각 모드의 동작 규칙을 확인할 때 읽는다.
...
> 각 단계 세부 절차는 `../skills/catalog-update/references/processing.md` 참조 — 각 B-N 단계를 실행하기 직전에 읽는다.

> 출력 형식·`--all` 집계·연계 흐름 순서 예시는 `../skills/catalog-update/references/output.md` 참조 — 결과를 출력하거나 다른 스킬과 연계할 때 읽는다.
...
8. **handbook 파일 수정 금지** — 이 에이전트는 모든 handbook 항목을 읽기 전용으로만 사용한다. handbook 파일 갱신은 `handbook-update-core` 에이전트에서 수행한다
9. `--all` 순회 시 `_wiki/` 없는 프로젝트는 건너뛰고 집계에 표시한다
10. **git commit/push 금지**: 에이전트는 파일 편집만 수행한다. 커밋은 `COMMIT_REQUESTED` 신호를 출력하고, 실제 실행은 호출자(래퍼 SKILL.md)가 담당한다.
