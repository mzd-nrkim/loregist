...
| `--all` | 전 프로젝트 순회 (stashdex project list --json 기반) |
...
`stashdex project list --json`으로 `handbook`에서 `writable: true` 항목만 추출한다. `--file` 지정 시 해당 파일의 writable 여부를 확인한다.
...
`--defer-embed` 없으면 반영된 파일을 `LOREGIST_AUTO_GUARD=1 stashdex embed --file …`로 직접 embed한다. `--defer-embed` 있으면 `EMBED_FILES: path1 path2 …` 한 줄을 출력하고 호출자에 위임한다. 갱신 0건·`--dry-run` 시 건너뛴다.
