...
1. `loregist projects --json` 파싱 → `docs_root != null` 프로젝트 목록 확보; `PERSONAL_WORK_ROOT` = `personal-work` 프로젝트의 `docs_root` 값
   ```bash
...
- [ ] 항목1  ([원본 인덱스](../projects/project-a/dev/{date}/{date}.01.작업문서.md))
- [ ] 항목2
...
## 실행 절차
1. 날짜 결정 (인자 또는 오늘)
2. `loregist projects --json` 실행, 파싱; `PERSONAL_WORK_ROOT` 결정 (위 동작 1단계 참조)
3. 대상 프로젝트 순회:
...
4. `loregist projects --json` 실패 시 에러 출력 후 중단
