...
- stale 판정 기준: 코드베이스와 명백히 불일치하거나 사실적 오류인 내용만 stale로 판정한다. 단순히 내용이 부족하거나 예시가 없는 섹션은 stale 판정에서 제외한다. 대조 소스에는 `.claude/skills/` 아래 SKILL.md 및 references/ 파일을 포함한다 — 문서 내 스킬 역할 설명이 실제 정의와 다를 경우 오류로 판정한다.
