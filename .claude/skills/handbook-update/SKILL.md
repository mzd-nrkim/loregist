...
> 인수 3축 의미·함의 규칙·대표 조합 매트릭스·`--now <이름>` 필터 동작·`--file`과 `--now` 병존 규칙(D1) 세부는 `references/cli.md` 참조 — 인수 해석이 모호하거나 조합 동작을 확인할 때 읽는다.

---

# 실행 흐름 (W-1 ~ W-9)

아래는 각 단계의 한 줄 요약이다. 판정 세부·분기 규칙 전체는 `references/gates.md` 참조 — 각 단계 진입 시 해당 섹션을 읽는다.
...
`writable: true`로 선언됐지만 디스크에 없는 파일을 코드베이스 분석으로 초안 작성 후 사용자 승인을 받아 Write한다. 기본 증분 모드에서는 이 단계를 건너뛴다.

## W-7. 섹션 단위 Edit 제안 + 사용자 승인 흐름

각 stale 섹션에 대해 현재 내용·제안 내용을 제시하고 사용자 응답(예/아니요/모두 승인/프로젝트 모두 승인/모두 취소)을 처리한다. `--dry-run` 시 목록만 출력하고 종료한다. `auto_handbook_update` 플래그가 켜진 프로젝트는 승인 프롬프트를 생략한다.

## W-8. `.last_handbook_update` 갱신

승인 반영이 1건 이상인 경우에만 `_wiki/.last_handbook_update`에 HEAD SHA(git repo) 또는 ISO 8601 타임스탬프(비git)를 기록한다.

> W-1 ~ W-8 판정 세부 전체는 `references/gates.md` 참조.

## W-9. embed 단계 (자기완결 embed / defer 분기)

`--defer-embed` 없으면 반영된 파일을 `LOREGIST_AUTO_GUARD=1 loregist embed --file …`로 직접 embed한다. `--defer-embed` 있으면 `EMBED_FILES: path1 path2 …` 한 줄을 출력하고 상위 wiki-update에 위임한다. 갱신 0건·`--dry-run` 시 건너뛴다.

> W-9 세부(재귀 차단·`--all` 합산 규칙)는 `references/gates.md` 참조.

> 출력 형식(단일/`--all`/`--commit`) 및 연계 흐름은 `references/output.md` 참조 — 완료 출력 작성 또는 `--commit` 커밋 실행 시 읽는다.
