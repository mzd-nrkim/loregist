...
`--all` 인자가 있으면 `stashdex project list --json` 결과 배열에서 전체 프로젝트 목록과 각 `docs_root`를 추출한다.
...
2.5. **병합 단락 재read (복수 절 병합 시에만 적용)**: 에이전트가 복수 절을 병합해 하나로 쓴 단락이 있는 경우, 해당 파일의 병합 단락 줄 범위를 `grep -n` 또는 `Read offset/limit`으로 재독한다. 확인 대상: 같은 단어/어절 2회 이상 반복·동일 의미 문장 연속. 단순 내용 추가·삭제 편집에는 해당 없음.
