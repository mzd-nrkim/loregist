...
# LOREGIST_PROJECTS_FILE 환경변수로 projects.toml 위치를 명시해 경로 mismatch를 방지.
...
def _build_blocked_paths():
    ...
    """
    try:
        from loregist.config import PROJECTS, get_readonly_handbook_paths
    except Exception as e:
        print(
            "[block_readonly_handbook] loregist.config 임포트 실패 (fail-open): {}".format(e),
            file=sys.stderr,
        )
        return set()
...
def main():
    ...
    # C-3 스코핑: 자동 무인 실행(loregist가 spawn한 claude)만 차단 대상.
    # LOREGIST_AUTO_GUARD는 auto_update.launch_headless()가 자식 env에만 주입한다.
    ...
    if is_readonly_handbook_path(file_path, blocked_paths):
        normalized = os.path.realpath(file_path)
        print(
            "[loregist] 차단: '{}' 은 writable=false handbook 파일입니다.\n"
            "  정규화 경로: {}\n"
