...
stashdex는 두 부분입니다. **본체**는 기록을 쌓고 의미로 검색하는 CLI이고, **스킬**은 그 검색을 활용해 문서를 쓰고 갱신하는 Claude Code 도구입니다. 스킬이 써낸 문서는 다시 기록이 되어 검색에 잡힙니다 — 둘은 한 바퀴 도는 순환입니다.
...
설치가 끝난 뒤 Claude Code를 열고 자연어로 말하면 됩니다.

```
어제 결정한 내용 검색해서 정리해줘
```

Claude가 `stashdex search`로 관련 기록을 찾아 결과를 정리합니다. 슬래시 명령어 없이도 스킬이 자동으로 호출됩니다.

> 이 동선이 동작하려면 각 프로젝트 CLAUDE.md에 검색 규칙 블록이 필요합니다. → [Claude Code 통합 설정](docs/public/USAGE.md#claude-code-통합-설정)
...
AI 도구 용어에 익숙하다면, stashdex는 이렇게 위치 지을 수 있습니다.
